CLASS zsa_cl_generate_data DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS Zsa_CL_GENERATE_DATA IMPLEMENTATION.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM zsa_d_prod_group.
*   insert the new table entries
    INSERT zsa_d_prod_group FROM TABLE @lt_prod_grs.

*   check the result
    SELECT * FROM zsa_d_prod_group INTO TABLE @lt_prod_grs.
    out->write( sy-dbcnt ).
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM zsa_d_phase.
*   insert the new table entries
    INSERT zsa_d_phase FROM TABLE @lt_phases.

*   check the result
    SELECT * FROM zsa_d_phase INTO TABLE @lt_phases.
    out->write( sy-dbcnt ).
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM zsa_d_market.
*   insert the new table entries
    INSERT zsa_d_market FROM TABLE @lt_markets.

*   check the result
    SELECT * FROM zsa_d_market INTO TABLE @lt_markets.
    out->write( sy-dbcnt ).
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM zsa_d_uom.
*   insert the new table entries
    INSERT zsa_d_uom FROM TABLE @lt_uom.
